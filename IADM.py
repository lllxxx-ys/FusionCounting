...
class FusionModel(nn.Module):
    ...
    def forward(self, RGBT):
        ...
        RGB, T, shared1 = self.block1(RGB, T, None)
        RGB, T, shared2 = self.block2(RGB, T, shared1)
        RGB, T, shared3 = self.block3(RGB, T, shared2)
        RGB, T, shared4 = self.block4(RGB, T, shared3)
        _, _, shared5 = self.block5(RGB, T, shared4)
        x = shared5

        # =============
        z = self.fusion_(shared1, shared2, shared3, shared4)
        z = self.model(z)
...
class Block(nn.Module):
    ...
    def forward(self, RGB, T, shared):
        ...
        device = RGB.device
        if self.first_block:
            shared = torch.zeros(RGB.shape, device=device)
        else:
